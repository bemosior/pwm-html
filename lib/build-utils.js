...
export function renderImage(href, title, text, durations = {}) {
  ...
  if (href.endsWith('.mp4')) {
    ...
    const filename = decodeURIComponent(originalHref.split('/').pop());
    const duration = durations[filename];
    const durationAttr = duration ? ` defaultduration="${duration}"` : '';
    return `<media-controller${durationAttr}><video slot="media" preload="none"${posterAttr}><source src="${href}" type="video/mp4">${text}</video><media-control-bar><media-play-button></media-play-button><media-time-range></media-time-range><media-time-display showduration></media-time-display><media-mute-button></media-mute-button><media-volume-range></media-volume-range><media-playback-rate-button></media-playback-rate-button><media-fullscreen-button></media-fullscreen-button></media-control-bar></media-controller>`;
  }
...
export function buildSidebar(currentLesson, sections) {
  let html = '<nav class="course-nav"><div class="nav-scroll">';
  ...
  html += '</div>';
  html += '<div class="nav-footer"><small><a href="mailto:ben+pwmsupport@hiredthought.com" style="text-decoration: none;">✉️ Support</a></small></div>';
  html += '</nav>';
  return html;
}
